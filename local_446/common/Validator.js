...
sap.ui.define([
     ...
     * @param {boolean} bResetValueState - Flag to remove all validation errors in control
     * @return {boolean} whether the oControl is valid or not.
     */
    Validator.prototype.validate = function (oControl, bResetValueState) {
        this._isValid = true;
        sap.ui.getCore().getMessageManager().removeAllMessages();
        this._validate(oControl, bResetValueState);
        return this.isValid();
    };
     ...
     * @param {boolean} bResetValueState - Flag to remove all validation errors in control
     */
    Validator.prototype._validate = function (oControl, bResetValueState) {
                        ...
                        if (oControl.getBinding(aValidateProperties[i]).getType()) {
                            ...
                            if (editable) {
                                ...
                                try {
                                    ...
                                    bResetValueState ? oControlBinding.getType().validateValue(bResetValueState) : oControlBinding.getType().validateValue(oInternalValue);
                        ...
                        if (aControlAggregation) {
                            ...
                            if (aControlAggregation instanceof Array) {
                                for (j = 0; j < aControlAggregation.length; j += 1) {
                                    this._validate(aControlAggregation[j], bResetValueState);
                                }
                            }
                            // ...however, with sap.ui.layout.form.Form, it is a single object *sigh*
                            else {
                                this._validate(aControlAggregation, bResetValueState);
                            }
                        }
                    }
                }
